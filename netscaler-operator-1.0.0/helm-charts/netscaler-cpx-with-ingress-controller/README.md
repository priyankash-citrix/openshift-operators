...
Toleration can be applied to pod running NetScaler CPX and ingress controller containers using `tolerations` argument while deploying CPX+NIC using helm chart. This argument takes list of tolerations that user need to apply on the CPX+NIC pods.

For example, following command can be used to apply toleration on the CPX+NIC pod:
...
| nic.imageRegistry                   | Mandatory  |  `quay.io`               |  The NetScaler ingress controller image registry             |  
| nic.imageRepository                 | Mandatory  |  `citrix/citrix-k8s-ingress-controller`              |   The NetScaler ingress controller image repository             | 
| nic.imageTag                  | Mandatory  |  `1.33.4`               |   The NetScaler ingress controller image tag            | 
| nic.pullPolicy | Mandatory | IfNotPresent | The NetScaler ingress controller image pull policy. |
| nic.required | Mandatory | true | NIC to be run as sidecar with NetScaler CPX |
| nic.resources | Optional | {} |	CPU/Memory resource requests/limits for NetScaler Ingress Controller container |
| nic.rbacRole  | Optional |  false  |  To deploy NIC with RBAC Role set rbacRole=true; by default NIC gets installed with RBAC ClusterRole(rbacRole=false)) |
...
| replicaCount  | Optional  | 1 | Number of CPX-NIC pods to be deployed. With `cpxBgpRouter : true`, replicaCount is 1 since CPX will be deployed as DaemonSet |
...
| nodeSelector.key | Optional | N/A | Node label key to be used for nodeSelector option for CPX-NIC deployment. |
| nodeSelector.value | Optional | N/A | Node label value to be used for nodeSelector option in CPX-NIC deployment. |
...
| tolerations | Optional | N/A | Specify the tolerations for the CPX-NIC deployment. |
